...
if __name__ == '__main__':
    ...
    target = Variable(torch.sin(input) + torch.rand(input.size()))
    ...
    mu = 0.01 # Lip penaly parameter 
    lmbd = 0.01 # L2 penalty parameter
    it_ADMM = 100
    ind_Lip = 2 # 1 Lipschitz regularization, 2 Enforcing Lipschitz bounds
    ...
    sin=torch.sin(input)+0.5

    torch.manual_seed(3)
    target_test = torch.sin(input) + torch.rand(input.size())
    ...
    data={
        ...
        'MSELoss_sin': net.evaluate_MSELoss(input,sin).detach().numpy(),
        'MSELoss_sin_L2': net_L2.evaluate_MSELoss(input,sin).detach().numpy(),
        'MSELoss_sin_Lip': net_Lip.evaluate_MSELoss(input,sin).detach().numpy(),
        'MSELoss_sin_Lip2': net_Lip2.evaluate_MSELoss(input,sin).detach().numpy(), 
